...
static sock_t mg_open_listening_socket(union socket_address *sa, int type,
  ...
  if ((sock = socket(sa->sa.sa_family, type, proto)) != INVALID_SOCKET &&
      ...
      !setsockopt(sock, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (const char*)((void *) &on),
                  sizeof(on)) &&
#endif
  ...
  if ((err = RegOpenKeyExW(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &hKey)) !=
    ...
    for (ret = -1, i = 0; 1; i++) {
      ...
      if (RegOpenKeyExW(hKey, subkey, 0, KEY_READ, &hSub) == ERROR_SUCCESS &&
          ((RegQueryValueExW(hSub, L"NameServer", 0, &type, (LPBYTE)((void *) value),
                             &len) == ERROR_SUCCESS &&
            value[0] != '\0') ||
           (RegQueryValueExW(hSub, L"DhcpNameServer", 0, &type, (LPBYTE)((void *) value),
                             &len) == ERROR_SUCCESS &&
            value[0] != '\0'))) {
        /*
