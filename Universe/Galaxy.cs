...
namespace Universe
{
    public class Galaxy
    {
        ...
        FixedVector minExtent, maxExtent;
        ...
        {
            minExtent = FixedVector.zero;
            maxExtent = FixedVector.zero;
            ...
            FixedVector extent = maxExtent - minExtent;
            ...
            int xMax = (int)Fixed.Ceiling(extent.x / cellSize);
            int yMax = (int)Fixed.Ceiling(extent.y / cellSize);
            int zMax = (int)Fixed.Ceiling(extent.z / cellSize);
            ...
            for (int x = 0; x < xMax; x++)
                for (int y = 0; y < yMax; y++)
                    for (int z = 0; z < zMax; z++)
                    {
                        ...
                        FixedVector boundsMin = minExtent + FixedVector.Create(x * cellSize, y * cellSize, z * cellSize);
                        FixedVector boundsMax = boundsMin + FixedVector.Create(cellSize, cellSize, cellSize);
                        ...
                        foreach (var star in Stars)
                            ...
                            else
                            {
                                ...
                                FixedVector closest = ClosestPoint(star.Position, boundsMin, boundsMax);
                                Fixed distance = FixedVector.Distance(closest, star.Position);
                                Fixed angularDiameter = 2 * star.Radius / distance;
        ...
        public RenderCell GetRenderCell(FixedVector location)
        {
            location -= minExtent;
        ...
        private bool Within(FixedVector pos, FixedVector boundsMin, FixedVector boundsMax)
        {
            for (int i = 0; i < 3; i++)
        ...
        private FixedVector ClosestPoint(FixedVector pos, FixedVector boundsMin, FixedVector boundsMax)
        {
            FixedVector vec = FixedVector.zero;
