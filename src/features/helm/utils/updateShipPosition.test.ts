...
import { updateShipMotion, shouldUpdateMotion, addWaypoint } from './updateShipMotion';
...
test('moves to single waypoint then holds position', () => {
    ...
    let currentTime = 0;
    expect(shouldUpdateMotion(ship, currentTime))
        .toBeTruthy();
    
    updateShipMotion(ship, currentTime);
