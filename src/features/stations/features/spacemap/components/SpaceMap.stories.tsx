...
export const appearances = () => {
  ...
  const canvasRefs = useRef<FakeRef<HTMLCanvasElement>[]>(
    new Array(Object.keys(drawFunctions).length).fill(0).map(() => ({ current: null }))
  );
  ...
  return <CanvasContainer>{canvases}</CanvasContainer>;
};
