import { clampAngle, Vector2D } from '../types/Vector2D';
import { Position } from '../types/Position';
import { durationToTicks, getCompletedFraction, getTime } from './timeSpans';

const endTicks = durationToTicks(3);
...
function getCurrentSegment<T>(animation: Animation<T>, currentTime: number): AnimationSegment<T> {
    ...
    if (firstFutureIndex >= 2) {
        if (animation.length >= firstFutureIndex + 2) {
            ...
            keepFrames.push({
                time: firstFuture.time + endTicks,
                val: firstFuture.val,
            });

            if (keepFrames.length !== 4) {
                keepFrames.push({
                    time: firstFuture.time + endTicks + endTicks,
                    val: firstFuture.val,
                });
            }
        ...
        return [
            ...
            {
                time: currentTime + endTicks + endTicks,
                val,
            },
        ]
    }
    ...
    if (firstFutureIndex === 1) {
        ...
        returnVal = [
            {
                time: firstVal.time - endTicks - endTicks,
                val: firstVal.val,
            },
            {
                time: firstVal.time - endTicks,
